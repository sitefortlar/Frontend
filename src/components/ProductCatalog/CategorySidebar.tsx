...
import { ChevronDown, ChevronRight, Menu, X, ShoppingBag, ClipboardList, LogIn } from 'lucide-react';
...
export const CategorySidebar = ({
  ...
  const sidebarContent = (
    <div className="h-full w-full flex flex-col" style={{ background: 'var(--gradient-sidebar)' }}>
      ...
      {/* Rodapé: pedidos + logout (autenticado) ou entrar (visitante) */}
      <div className="p-4 border-t border-sidebar-border flex-shrink-0 space-y-2">
        ...
        {isAuthenticated ? (
          ...
          <Button
            variant="ghost"
            className="w-full justify-start text-sidebar-foreground hover:bg-sidebar-accent/30"
            asChild
          >
            <Link to={paths.login} onClick={() => setIsMobileOpen(false)}>
